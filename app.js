  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (SITE_CONFIG.formEndpoint) {
        ...
        const findNativeField = (matcher) => [...nativeForm.querySelectorAll('input, textarea')].find((field) => {
          const meta = [field.name, field.type, field.dataset.tildaRule, field.dataset.tildaFld].filter(Boolean).join(' ').toLowerCase();
          return matcher(meta);
        });
